class AddPointsState extends BaseState {
	...
	update() {
		for (let i = 0; i < this.points.length; i++) {
			...
			if (MOUSE_PRESSED && distance(mouseX, mouseY,CONSTRUCTIONS[i].x,CONSTRUCTIONS[i].y) <= this.select_radius) STATEMACHINE.change("move_point", {selected: points[i], previousState: this.name});
		}
